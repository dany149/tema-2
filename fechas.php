...
// Función para obtener las entradas paginadas con orden
function obtenerEntradasPaginadas($pagina, $porPagina, $orden)
{
    $conexion = obtenerConexion();
    $inicio = ($pagina - 1) * $porPagina;
    $stmt = $conexion->prepare("SELECT * FROM Entradas ORDER BY fecha_creacion $orden LIMIT :inicio, :porPagina");
...
<body>
    ...
    <table>
        <tr>
            ...
            <th>Contenido</th>
            <th>
                <!-- Agregar enlace para ordenar por fecha -->
                <a href="?pagina=<?php echo $paginaActual; ?>&orden=asc">Fecha de Creación ▲</a>
                <a href="?pagina=<?php echo $paginaActual; ?>&orden=desc">Fecha de Creación ▼</a>
            </th>
            <th>ID Usuario</th>
            <th>Imagen</th>
        </tr>
        ...
        <?php foreach ($entradas as $entrada): ?>
            <tr>
                ...
                <td>
                    <?php echo $entrada['fecha_creacion']; ?>
                </td>
                <td>
                    <?php echo $entrada['id_usuario']; ?>
                </td>
                <td>
    ...
    <div class="pagination">
        <span>Página
            <?php echo $paginaActual; ?> de
            <?php echo $totalPaginas; ?>
        </span>
        <?php if ($paginaActual > 1): ?>
            <a href="?pagina=<?php echo $paginaActual - 1; ?>&orden=<?php echo $orden; ?>">&laquo; Anterior</a>
        <?php endif; ?>
        <?php for ($i = 1; $i <= $totalPaginas; $i++): ?>
            <a href="?pagina=<?php echo $i; ?>&orden=<?php echo $orden; ?>" <?php echo ($i == $paginaActual) ? 'class="active"' : ''; ?>>
                <?php echo $i; ?>
            </a>
        <?php endfor; ?>
        <?php if ($paginaActual < $totalPaginas): ?>
            <a href="?pagina=<?php echo $paginaActual + 1; ?>&orden=<?php echo $orden; ?>">Siguiente &raquo;</a>
        <?php endif; ?>
    </div>
